...
async def async_setup_entry(hass, config_entry, async_add_entities: AddEntitiesCallback):
    ...
    for client in clients:
        mac = client.get('MAC')
        if not mac:
            _LOGGER.warning(f"Client {client.get('hostname', 'Unknown')} does not have a MAC address. Skipping.")
            continue
...
class OmadaClientEntity(CoordinatorEntity, ScannerEntity):
    ...
    def name(self):
        ...
        return self.client_info.get('hostname', 'Unknown Device')

    @property
    def unique_id(self):
        """Return a unique ID for the client."""
        mac = self.client_info.get('MAC', 'unknown').lower().replace(':', '').replace('-', '')
    ...
    def device_info(self):
        ...
        if self.access_point_mac and self.access_point_mac != 'unknown':
            return DeviceInfo(
                identifiers=set(),
                connections={(CONNECTION_NETWORK_MAC, self.access_point_mac)},  # Associate with access point
            )
        else:
